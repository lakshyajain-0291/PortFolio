...
import { Github, Star, GitFork, Calendar, Code, Activity, Users, Award, Zap } from 'lucide-react';
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const extractedUsername = extractGithubUsername(data?.socialLinks?.github);
        setUsername(extractedUsername);
        
        if (!extractedUsername) {
          console.error('GitHub username could not be extracted from portfolio data');
      ...
      <section id="github-stats" className="py-20 relative">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="glass-panel p-6">
            <p className="text-darktech-cyber-pink mb-4">No GitHub statistics available. Connect your GitHub account to view your coding activity.</p>
    ...
    <section id="github-stats" className="py-20 relative">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center mb-12">
          <div className="glass-panel p-4 rounded-xl border border-darktech-border hover:border-darktech-neon-green/30 transition-all">
            <div className="flex items-center justify-center mb-2">
              <Github size={28} className="text-darktech-neon-green" />
            </div>
            <p className="text-3xl font-bold">{portfolioData.githubStats.totalPublicRepos}</p>
            <p className="text-sm text-darktech-muted">Repositories</p>
          </div>
          
          <div className="glass-panel p-4 rounded-lg border border-darktech-border hover:border-darktech-neon-green/50 transition-all">
            <div className="flex items-center justify-center mb-2">
              <Star size={28} className="text-yellow-400" />
            </div>
            <p className="text-3xl font-bold">{portfolioData.githubStats.totalStars}</p>
          ...
          <div className="glass-panel p-4 rounded-lg border border-darktech-border hover:border-darktech-neon-green/50 transition-all">
            ...
            <p className="text-3xl font-bold">{portfolioData.githubStats.totalForks}</p>
          ...
          <div className="glass-panel p-4 rounded-lg border border-darktech-border hover:border-darktech-neon-green/50 transition-all">
            ...
            <p className="text-3xl font-bold">{portfolioData.githubStats.totalCommits}</p>
            <p className="text-sm text-darktech-muted">Commits</p>
          </div>
        </div>

        {/* Contribution Graph - Full Width */}
        <div className="glass-panel p-6 rounded-xl border border-darktech-border hover:border-darktech-neon-green/30 transition-all mb-12">
          ...
          <div className="flex justify-between px-2 mt-3">
            ...
            <span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor: 'rgba(0, 128, 0, 1)'}}></span>
            <span className="text-xs text-darktech-muted pl-2">Less</span>
            </div>
            <div className="flex items-center gap-1">
              <span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor: 'rgba(0, 128, 0, 0.8)'}}></span>
              <span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor: 'rgba(0, 128, 0, 0.6)'}}></span>
              <span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor: 'rgba(0, 128, 0, 0.4)'}}></span>
              <span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor: 'rgba(0, 128, 0, 0.2)'}}></span>
            </div>
            <div className="flex items-center">
            <span className="inline-block w-3 h-3 rounded-sm" style={{backgroundColor: 'rgba(0, 128, 0, 0.2)'}}></span>
