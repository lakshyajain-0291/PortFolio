...
import { Github, ExternalLink, Loader2, Star, GitFork, Calendar, Search, Filter, ChevronDown, ChevronUp } from 'lucide-react';
...
const getRepoImageUrl = (githubUrl?: string) => {
  if (!githubUrl) return DEFAULT_ASSETS.PROJECT_IMAGE || '/assets/default-project.png';
  
  // Extract the username and repo name from the GitHub URL
  const match = githubUrl.match(/github\.com\/([^\/]+)\/([^\/]+)/);
  if (!match) return DEFAULT_ASSETS.PROJECT_IMAGE || '/assets/default-project.png';
...
const Projects = () => {
  ...
  const enhanceProject = (project: EnhancedProject): EnhancedProject => {
    ...
    const repoData = project.stars !== undefined && project.forks !== undefined ? {
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
    e.currentTarget.src = DEFAULT_ASSETS.PROJECT_IMAGE || '/assets/default-project.png';
  };
      ...
      <section id="projects" className="py-20 relative">
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="glass-panel p-6">
            <p className="text-darktech-cyber-pink mb-4">No projects found. Connect your GitHub account to showcase your work.</p>
  ...
  const noMatchingProjects = filteredProjects.length === 0 && allProjects.length > 0;
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <p className="text-darktech-muted max-w-2xl mx-auto">
            Check out my latest work, ranging from web applications to AI integrations and game development.
          </p>
        </div>
        ...
        <div className="mb-12">
          ...
          {availableTech.length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 max-w-3xl mx-auto">
              <span className="flex items-center text-sm text-darktech-muted mr-2">
                <Filter size={14} className="mr-1" /> Filters:
              </span>
              
              {/* Show only first 5 filters by default */}
              {availableTech.slice(0, showAllTech ? availableTech.length : 5).map((tech) => (
                <Badge
                  key={tech}
                  variant={selectedTechFilter.includes(tech) ? "default" : "outline"}
                  className={`cursor-pointer hover:bg-darktech-card transition-colors ${
                    selectedTechFilter.includes(tech) ? "bg-darktech-neon-green text-darktech-background" : ""
                  }`}
        ...
        {noMatchingProjects && (
          <div className="glass-panel p-6 text-center mb-12">
            <p className="text-darktech-cyber-pink">No projects match your filters. Try adjusting your search criteria.</p>
          </div>
        )}
        ...
        {!noMatchingProjects && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProjects.slice(0, showMoreProjects ? undefined : 6).map((project, index) => (
              <div 
                ...
                <div className="relative overflow-hidden h-48">
                  ...
                  {project.repoData && (
                    <div className="absolute top-3 right-3 flex items-center space-x-3">
                      <div className="flex items-center gap-1 bg-darktech-background/80 px-3 py-1.5 rounded-full backdrop-blur-sm" title="GitHub Stars">
                        <Star size={16} className="text-yellow-400" />
                        <span className="text-sm font-medium">{project.repoData.stars.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center gap-1 bg-darktech-background/80 px-3 py-1.5 rounded-full backdrop-blur-sm" title="GitHub Forks">
                        <GitFork size={16} className="text-green-400" />
                        <span className="text-sm font-medium">{project.repoData.forks.toLocaleString()}</span>
                      </div>
                    </div>
                  )}
        ...
        {filteredProjects.length > 6 && (
          <div className="flex justify-center mt-10">
            ...
            >
              {showMoreProjects ? 'Show Less' : `Show ${filteredProjects.length - 6} More Projects`}
            </button>
          </div>
        )}
      </div>
